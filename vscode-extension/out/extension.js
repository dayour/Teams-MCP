...
exports.TeamsMCPExtension = void 0;
...
class TeamsMCPExtension {
    ...
    async activate() {
        ...
        if (config.get('autoStart')) {
            ...
            if (await this.isAuthenticated()) {
                ...
                vscode.window.showInformationMessage('Teams MCP: Please authenticate to start the server automatically.', 'Configure').then(selection => {
    ...
    async setupDeviceAuth() {
        ...
        vscode.window.showInformationMessage('Starting device authentication flow. You will be prompted to sign in to Microsoft.', 'Continue').then(async (selection) => {
            if (selection === 'Continue') {
                try {
                    await this.initiateDeviceFlow();
                    vscode.window.showInformationMessage('Authentication successful! Teams MCP is now configured.', 'Start Server').then(selection => {
    ...
    async setupAppRegistration() {
        ...
        vscode.window.showInformationMessage('Azure app registration configured successfully!', 'Start Server').then(selection => {
            if (selection === 'Start Server') {
                this.startServer();
            }
        });
    ...
    async startServer() {
        ...
        try {
            ...
            if (!fs.existsSync(serverPath)) {
                vscode.window.showErrorMessage('MCP server not found. Please rebuild the extension.', 'Rebuild').then(selection => {
            ...
            vscode.window.showErrorMessage(`Failed to start Teams MCP server: ${error instanceof Error ? error.message : String(error)}`);
            this.updateStatusBar('Error');
        }
    }
    async stopServer() {
...
• View Claude Desktop config
        `.trim();
        const panel = vscode.window.createWebviewPanel('teamsMcpStatus', 'Teams MCP Status', vscode.ViewColumn.One, {});
        panel.webview.html = `
            <!DOCTYPE html>
            <html>
            <head>
    ...
    async updateClaudeConfig(configPath) {
        try {
            let config = {};
            if (fs.existsSync(configPath)) {
            ...
            vscode.window.showInformationMessage('Claude Desktop configuration updated! Restart Claude Desktop to use Teams MCP.', 'Open Config').then(selection => {
    ...
    async ensureAuthenticated() {
        if (!(await this.isAuthenticated())) {
            const choice = await vscode.window.showInformationMessage('Teams MCP is not configured. Would you like to set it up now?', 'Configure', 'Later');
            if (choice === 'Configure') {
